...
function render(){ if(!DATA) return; ({inicio:renderInicio, programa:renderPrograma, agenda:renderAgenda, eventos:renderEventos, carteles:renderCarteles, constancias:renderConstancias, info:renderInfo}[view]||renderInicio)(); }
function daysLeft(){ const target=new Date(DATA.evento.fechaInicio+'T09:00:00'); const now=new Date(); return Math.max(0,Math.ceil((target-now)/(1000*60*60*24))); }
...
function renderInicio(){
  app.innerHTML = `<div class="layout"><div>
    <section class="hero">
      <div class="kicker">Programa académico</div>
    ...
    <section class="stats">
      <div class="stat"><b>${daysLeft()}</b><span>días para el congreso</span></div>
  ...
  </div><aside class="side-col">${noticesCard()}${constanciaMini()}${cartelesMini()}</aside></div>`;
}
function todayRows(){
  const rows=[
...
function filterHeader(title,lead,onlyType){
 ...
 <div class="toolbar"><input id="programSearch" value="${filters.q||''}" placeholder="Buscar por título, autor, institución o código…" oninput="filters.q=this.value;render()"><select onchange="filters.mesa=this.value;render()">${mesas.map(m=>`<option ${filters.mesa===m?'selected':''} value="${m}">${m==='all'?'Todas las mesas':m}</option>`).join('')}</select><select onchange="filters.modalidad=this.value;render()"><option value="all">Todas</option><option ${filters.modalidad==='Presencial'?'selected':''}>Presencial</option><option ${filters.modalidad==='Virtual'?'selected':''}>Virtual</option></select></div>`;
}
...
function renderConstancias(){
 const q=(filters.q||'').toLowerCase(); const items=DATA.constancias.filter(c=>!q||[c.nombre,(c.codigos||[]).join(' ')].join(' ').toLowerCase().includes(q));
 app.innerHTML=`<div class="section-kicker">Constancias</div><h1 class="section-title">Busca y descarga constancias</h1><p class="section-lead">Cuando estén listas, sube los PDF en <b>constancias/ponencias</b> o <b>constancias/carteles</b> y actualiza el nombre del archivo si es necesario.</p><div class="toolbar"><input value="${filters.q||''}" placeholder="Buscar por nombre, apellido o código…" oninput="filters.q=this.value;render()"></div>${items.length?`<div class="grid">${items.slice(0,120).map(c=>`<article class="card work-card"><div class="meta">${c.tipo}</div><h3>${c.nombre}</h3><p>${(c.codigos||[]).slice(0,4).join(', ')}${(c.codigos||[]).length>4?'…':''}</p><div class="card-actions"><a class="small-btn" href="${c.pdf}" target="_blank">Descargar PDF</a></div></article>`).join('')}</div>`:'<div class="empty">No hay constancias con esa búsqueda.</div>'}`;
}
