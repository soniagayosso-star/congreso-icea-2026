...
function render(){ if(!DATA) return; ({inicio:renderInicio, programa:renderPrograma, agenda:renderAgenda, eventos:renderEventos, carteles:renderCarteles, constancias:renderConstancias, info:renderInfo}[view]||renderInicio)(); updateCountdown(); }
function delayedRender(){ clearTimeout(searchTimer); searchTimer=setTimeout(()=>render(),220); }
...
function renderInicio(){
  app.innerHTML = `<div class="layout"><div>
    <section class="hero">
      ...
      <div id="countdownLive" class="countdown-live"></div><div class="hero-actions"><button class="btn primary" onclick="go('programa')">Ver programa</button><button class="btn ghost" onclick="go('agenda')">Mi agenda</button><button class="btn ghost" onclick="go('constancias')">Constancias</button></div>
    </section>
    <section class="stats">
...
function filterHeader(title,lead,onlyType){
 ...
 <div class="toolbar"><input id="programSearch" value="${filters.q||''}" placeholder="Buscar por título, autor, institución o código…" oninput="filters.q=this.value;delayedRender()"><select onchange="filters.mesa=this.value;render()">${mesas.map(m=>`<option ${filters.mesa===m?'selected':''} value="${m}">${m==='all'?'Todas las mesas':m}</option>`).join('')}</select><select onchange="filters.modalidad=this.value;render()"><option value="all">Todas</option><option ${filters.modalidad==='Presencial'?'selected':''}>Presencial</option><option ${filters.modalidad==='Virtual'?'selected':''}>Virtual</option></select></div>`;
}
...
function renderConstancias(){
 ...
 app.innerHTML=`<div class="section-kicker">Constancias</div><h1 class="section-title">Busca y descarga constancias</h1><p class="section-lead">Cuando estén listas, sube los PDF en <b>constancias/ponencias</b> o <b>constancias/carteles</b> y actualiza el nombre del archivo si es necesario.</p><div class="toolbar"><input value="${filters.q||''}" placeholder="Buscar por nombre, apellido o código…" oninput="filters.q=this.value;delayedRender()"></div>${items.length?`<div class="grid">${items.slice(0,120).map(c=>`<article class="card work-card"><div class="meta">${c.tipo}</div><h3>${c.nombre}</h3><p>${(c.codigos||[]).slice(0,4).join(', ')}${(c.codigos||[]).length>4?'…':''}</p><div class="card-actions"><a class="small-btn" href="${c.pdf}" target="_blank">Descargar PDF</a></div></article>`).join('')}</div>`:'<div class="empty">No hay constancias con esa búsqueda.</div>'}`;
}
